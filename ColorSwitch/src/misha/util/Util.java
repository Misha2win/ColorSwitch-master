...
public class Util {
	...
	public static <T> void insert(T[] arr, int oldIndex, int newIndex) {
		T itemToInsert = arr[oldIndex];
		
		boolean forwards = oldIndex > newIndex;
		int start = forwards ? newIndex : oldIndex;
		int end = forwards ? oldIndex : newIndex;
		
		for (int i = (forwards ? end : start); (forwards ? i > start : i < end); i = (forwards ? i - 1 : i  + 1)) {
			arr[i] = arr[(forwards ? i - 1 : i + 1)];
		}
		
		arr[newIndex] = itemToInsert;
	}
